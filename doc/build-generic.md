GENERIC BUILD NOTES
====================
Some notes on how to build SCC Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build SCC with system-wide installed dependencies
...
---------------------
SCC inherited the `depends` folder from Bitcoin, which contains all dependencies required to build SCC. These
dependencies must be built before SCC can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build SCC Core. Caching of build results will ensure that only
...
not done, build failures might occur when building SCC.
...
ccache
------
`./configure` of SCC Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
