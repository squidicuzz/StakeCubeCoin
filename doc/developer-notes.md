...
- [Development guidelines](#development-guidelines)
    - [General SCC Core](#general-scc-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
SCC Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
SCC Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_scc
...
# A coverage report will now be accessible at `./test_scc.coverage/index.html`.
```
...
To profile a running sccd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep sccd` -- sleep 60
```
...
SCC Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 40000.
...
However, in open source software such as SCC Core, where everyone uses
...
pay attention to for reviewers of SCC Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: SCC Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against SCC Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `scc-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `scc-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of SCC Core, the wallet was always
