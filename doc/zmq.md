...
The SCC Core daemon can be configured to act as a trusted "border
router", implementing the scc wire protocol and relay, making
...
The ZeroMQ feature in SCC Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building sccd:
...
For instance:
    ...
    $ sccd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/sccd.tx.raw \
...
These options can also be provided in stakecubecoin.conf.
...
From the perspective of sccd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into sccd directly. Furthermore, no information is
...
using. sccd appends an up-counting sequence number to each
