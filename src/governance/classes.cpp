...
bool CSuperblockManager::GetSuperblockPayments(CGovernanceManager& governanceManager, int nBlockHeight, std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    for (int i = 0; i < pSuperblock->CountPayments(); i++) {
        ...
        if (pSuperblock->GetPayment(i, payment)) {
            ...
            // TODO: PRINT NICE N.N SCC OUTPUT
...
CAmount CSuperblock::GetPaymentsLimit(int nBlockHeight)
{
    ...
    CAmount nSuperblockPartOfSubsidy = GetBlockSubsidy(nBlockHeight - 1);
    ...
    */
    ...
    for (int i = 0; i < (int)vecParsed1.size(); i++) {
        ...
        if (!IsValidDestination(dest)) {
            std::ostringstream ostr;
            ostr << "CSuperblock::ParsePaymentSchedule -- Invalid SCC Address : " << vecParsed1[i];
            LogPrintf("%s\n", ostr.str());
            throw std::runtime_error(ostr.str());
        }
