 ...
 */
#ifndef WIN32
static const char* BITCOIN_PID_FILENAME = "sccd.pid";
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<sccaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by SCC Platform, to the specified username.", ArgsManager::ALLOW_ANY, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/stakecube/StakeCubeCoin>";
    const std::string URL_WEBSITE = "<https://stakecube.net/>";
...
/** Sanity checks
 *  Ensure that SCC Core is running in a usable environment with all
...
{
    // Make sure only a single SCC Core process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if SCC Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if SCC Core is started while in a temporary directory.\n",
        ...
        do {
            ...
            try {
                ...
                if (!fDisableGovernance && !gArgs.GetBoolArg("-txindex", DEFAULT_TXINDEX)
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/stakecube/StakeCubeCoin/pull/1817 and https://github.com/stakecube/StakeCubeCoin/pull/1743
    ...
    // ********************************************************* Step 10c: schedule SCC-specific tasks
