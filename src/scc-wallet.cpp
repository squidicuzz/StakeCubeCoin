...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s scc-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "scc-wallet is an offline tool for creating and interacting with SCC Core wallet files.\n" +
                                      "By default scc-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n" +
